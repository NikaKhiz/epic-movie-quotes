...
const hasGanresChosen = computed(() => {
  return editMoviesStore.genres.length > 0;
});
...
const deleteFromGenres = (id) => {
  const newGenres = editMoviesStore.genres.filter((item) => item.id !== id);
  editMoviesStore.genres = newGenres;
};
  ...
  >
    <div class="flex items-center gap-2" @click.prevent="toggleShowGenres">
      ...
      <div class="flex items-center gap-2" v-else>
        <div
          v-for="genre in editMoviesStore.genres"
          :key="genre"
      ...
      <li v-for="genre in props.genres" :key="genre.id">
        <label :for="`genre-${genre.id}`">
          <Field
            ...
            :value="genre"
            :rules="rules"
            v-model="editMoviesStore.genres"
          />
